...
def ingest_all_by_version(version_tag: str = "v0.2", source_dir: str = RAW_FILES_DIR, reupload: bool = False):
    ...
    for parquet_path in base_path.rglob("*.parquet"):
        ...
        process_single_nuts.delay(version_tag, str(parquet_path), reupload)
        found_files += 1
...
@celery_app.task(soft_time_limit=3600)
def process_single_nuts(version_tag: str, file_path: str, reupload: bool = False):
    """Handles one NUTS file: direct binary upload and conversion to GeoPackage, Shapefile, etc."""
